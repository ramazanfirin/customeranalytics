...
//    public static class TFacePositionasdasd extends com.sun.jna.Structure {
//    	
//    }
    ...
    public static class TFacePosition extends com.sun.jna.Structure {
		...
		protected List getFieldOrder() {
			...
			return Arrays.asList(new String[] { "xc", "yc", "w", "angle" });
		}
    }
    ...
    public static class FSDK_Features extends com.sun.jna.Structure {
		...
		protected List getFieldOrder() {
			// TODO Auto-generated method stub
			return Arrays.asList(new String[] {"features" });
		}
    }
    ...
    */
    ...
    private interface IFaceSDK extends Library {
        ...
        int FSDK_SaveImageToBuffer(int Image, byte[] Buffer, int ImageMode);
        ...
        int FSDK_PurgeID(int Tracker, long ID);
    ...
    public static int SaveImageToAWTImage(HImage Image, java.awt.Image DestImage[], int ImageMode){
        ...
        if (w[0] != 0 && h[0] != 0){
            res = IFaceSDK.INSTANCE.FSDK_SaveImageToBuffer(Image.himage, Buffer, ImageMode);
    ...
    public static int LoadImageFromBuffer(HImage Image, byte[] Buffer, int Width, int Height, int ScanLine, int ImageMode){
        IntByReference tmp = new IntByReference();
        int res = IFaceSDK.INSTANCE.FSDK_LoadImageFromBuffer(tmp, Buffer, Width, Height, ScanLine, ImageMode);
        Image.himage = tmp.getValue();
        return res;
    }
    public static int GetImageBufferSize(HImage Image, int BufSize[], int ImageMode){
        if (BufSize.length < 1)
            return FSDKE_INVALID_ARGUMENT;
        IntByReference tmp = new IntByReference();
        int res = IFaceSDK.INSTANCE.FSDK_GetImageBufferSize(Image.himage, tmp, ImageMode);
        if (res == FSDKE_OK)
            BufSize[0] = tmp.getValue();
        return res;
    }
    public static int SaveImageToBuffer(HImage Image, byte Buffer[], int ImageMode){
        return IFaceSDK.INSTANCE.FSDK_SaveImageToBuffer(Image.himage, Buffer, ImageMode); 
    }   
    ...
    public static int PurgeID(HTracker Tracker, long ID){
        return IFaceSDK.INSTANCE.FSDK_PurgeID(Tracker.htracker, ID);
    }
